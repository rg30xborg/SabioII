...
function Aviso2(){
	...
	else{	// window.defaultStatus = 'RG 3.0 - Sabichão Web [DADOS 1]'
...
function myBackCart(co){
	...
	if(cox == 'ok'){
		...
		alert('RG DADOS WEB ALERTA\n\nApostas [ ' + Math.abs(cred - apost) + ' Pontos ] acima do Total Atual disponível.\nAjuste e tente novamente....              Grato.')
	}
...
function coleta(){
	if(gparc > 0){
		...
		Reg17.innerText = 'Prêmios ' + gparc + ' Pontos - ' + premio; Reg20.src = '/_Library/botcg.gif'
		alocar1('play','sominici')}
	else{	Reg17.innerText = 'Rodada sem Prêmios - Sorry !'; Reg20.src = '/_Library/botlt.gif'
...
function ProssRGDadosWeb(){
	if(troc == 0){
		...
		Reg17.innerText = 'Faça suas apostas'; Reg16.innerText = 'Tabuleiro Pronto - Apostas Ok!'
...
function instrui(){
	...
	+ "  2a) click simples na região da aposta.\n"
	...
	+ "\n1:3 se Repetição de 2 Dados"
	+ "\n1:10 se Repetição de 3 Dados"
	+ "\n    Soma dos Três Dados:"
