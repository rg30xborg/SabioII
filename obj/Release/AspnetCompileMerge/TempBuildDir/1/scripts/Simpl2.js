...
function Aviso2(){
	...
	else{	// window.defaultStatus = 'RG 3.0 - Sabichão Web [POKER 1]'
		NCalc.innerText = " "; Reg16.innerText = "Defina o valor da Aposta Padrão"
	}
}
...
function myBackCart(co){
	...
	if(cred < aposm && cred != 0){aposm = cred
		alert('RG POKER WEB ALERTA:\n\nAposta ajustada para o limite de Pontos disponível')}
	if(cred != 0)
...
function filaOk1(){
	if(fila1x == 4){fila1x = 0; image1.style.visibility = "hidden"; image1.alt = "NÃO Descartar carta"
	...
	if(fila1x == 1){fila1x = 2; image1.src = '/_Library/cartaA.gif'
		Reg16.innerText = 'Três clicks na 1º Carta = Filar'; image1.alt = "1º Carta da Dobra"
...
function filaOk2(){
	if(fila2x == 4){fila2x = 0; image2.style.visibility = "hidden"; image2.alt = "NÃO Descartar carta"
	...
	if(fila2x == 1){fila2x = 2; image2.src = '/_Library/cartaA.gif'
		Reg16.innerText = 'Três clicks na 2º Carta = Filar'; image2.alt = "2º Carta da Dobra"
		Reg17.innerText = 'Palpite = ' 
		+ (dob == 'x1' ? (numero + naipe):(dob == '>' ? 'Maior que 7':'Menor que 7'))
	}
}
...
function filaOk3(){
	if(fila3x == 4){fila3x = 0; image3.style.visibility = "hidden"; image3.alt = "NÃO Descartar carta"
	...
	if(fila3x == 1){fila3x = 2; image3.src = '/_Library/cartaA.gif'
		Reg16.innerText = 'Três clicks na 3º Carta = Filar'; image3.alt = "3º Carta da Dobra"
		Reg17.innerText = 'Palpite = ' 
		+ (dob == 'x1' ? (numero + naipe):(dob == '>' ? 'Maior que 7':'Menor que 7'))
	}
}
...
function filaOk4(){
	if(fila4x == 4){fila4x = 0; image4.style.visibility = "hidden"; image4.alt = "NÃO Descartar carta"
	...
	if(fila4x == 1){fila4x = 2; image4.src = '/_Library/cartaA.gif'
		Reg16.innerText = 'Três clicks na 4º Carta = Filar'; image4.alt = "4º Carta da Dobra"
		Reg17.innerText = 'Palpite = ' 
		+ (dob == 'x1' ? (numero + naipe):(dob == '>' ? 'Maior que 7':'Menor que 7'))
	}
}
...
function filaOk5(){
	if(fila5x == 4){fila5x = 0; image5.style.visibility = "hidden"; image5.alt = "NÃO Descartar carta"
		image5.src = '/_Library/carta.gif'; Reg17.innerText = r
		cart = 0; Reg21.src = '/_Library/botcr.gif'
		if(lok_it == 'yes'){Broncyes(); alert('Ok!!!!, Você tentou, ...')}
		else{	Bronctno(); Reg26.src = '/_Library/botcv.gif'
			Reg24.src = '/_Library/botmv.gif'; Reg25.src = '/_Library/botmv.gif'
			alert('wall!!, Isto foi Uótimo.')}
	...
	if(fila5x == 1){fila5x = 2; image5.src = '/_Library/cartaA.gif'
		Reg16.innerText = 'Três clicks na 5º Carta = Filar'; image5.alt = "5º Carta da Dobra"
		Reg17.innerText = 'Palpite = ' 
		+ (dob == 'x1' ? (numero + naipe):(dob == '>' ? 'Maior que 7':'Menor que 7'))
	}
}
...
function instrui(){
	...
	+ (aposm * 10)  + " se Fulla (*).\n"
	+ (aposm * 6)   + " se Flash.\n"
	+ (aposm * 5)   + " se Sequência.\n"
	+ (aposm * 3)   + " se Trinca.\n"
	+ (aposm * 2)   + " se Duas Duplas.\n"
	+ "(*) Dobra o valor na presença de (As)." )
}

function infodobr(){
	alert("RG 3.0 - DOBRAS:\n" +
		"(Quando disponível permite aumentar o valor dos ganhos da Rodada).\n\n" +
		"1 - Dobra Maior/Menor que 7:\n" + 
		"    - Palpite certo = 2x os ganhos.\n" +
		"    - O Sete é neutro e mantém os ganhos.\n" +
		"2 - Dobra Cheia:\n" + 
		"    - Só naipe certo = 2x os ganhos.\n" + 
		"    - Só número certo = 4x os ganhos.\n" + 
		"    - Número e Naipe certo = 10x os ganhos.\n\n" +
...
function dobram(n){
	...
	else 	alert('GRUPO DE BOTÕES DOBRA (+/- 7) :\n\n- Disponíveis somente quando após (Trocar Cartas)\n   o resultado for maior ou igual a Duas Duplas.\n- Os botões liberados tem rótulo: [ - ] ou [ + ] .')
}
...
function cheiax(){
	...
	else 	alert('BOTÃO DOBRA CHEIA :\n\n- Disponível somente quando após (Trocar Cartas)\n   o resultado for maior ou igual a Duas Duplas.\n- O botão liberado tem rótulo: [ Cheia ] .')
}
