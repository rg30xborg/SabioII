...
Else
	...
	Function NewResposta(newIDPerg, Conexao)
		...
		Perguntas.Open "SELECT Resposta FROM Perg0001 WHERE Codigo=" & newIDPerg, Conexao
	...
	Function NewPergunta(IDPerg, conexao)
		Set Perguntas = Server.CreateObject("ADODB.Recordset")
		Perguntas.Open "SELECT * FROM Perg0001 WHERE Codigo=" & IDPerg, Conexao
	...
	Function LeSala(Conexao)
		...
		UsuarioDados.Open "SELECT USD_Total, USD_Consec, USD_Sala1 FROM UserDados WHERE USR_ID = " & Session("id"), Conexao
			text0 = UsuarioDados("USD_Sala1")
	...
	Function GravaSala(Conexao)
		Conexao.Execute "UPDATE UserDados SET USD_Total = " & pTotl & ", USD_Consec = " & pCosc & ",  USD_Sala1 = '" & text1 & "' WHERE USR_ID = " & Session("id")
	End Function
End If
%>
...
function Method1(apost){
	...
	text1 = resp0 + "Perg:" + newIDPerg + "Sala:" + pSala + "Data:" + Session("Hoje") + "Qtde:" + sQtde
...
function Method2(){
	...
	if(sQtde > Session("Dia") || IDPerg > Session("Dados")){
		pSala = parseInt(text0.substr(text0.indexOf("Sala:") + 5), 10)
		Conexao.Close()
		return new Array("xo", pSala, 1)
	}
	else{ perg1 = resp1 = resp2 = resp3 = resp4 = resp5 = figr1 = ""
		NewPergunta(IDPerg, Conexao)
		Conexao.Close()
		return new Array(perg1,resp1,resp2,resp3,resp4,resp5,figr1)
	}
}
