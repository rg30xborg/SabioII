...
' Numero de perguntas no banco de dados na Sala e o ("hoje") é para computar o dia na Sala
	...
	RS_Max.Open "SELECT MAX(Codigo) AS max_Codigo FROM Perg0001", Conexao
	...
	UsuarioDados.Open "SELECT USD_Consec, USD_Sala1 FROM UserDados WHERE USR_ID = " & Session("id"), Conexao
	pCosc = UsuarioDados("USD_Consec")
	text = UsuarioDados("USD_Sala1")   'dPerg:1Sala:0Data:10/04/2021Qtde:1
	If text <> "new" Then
		pResp = Left(text, 1)
		...
		Else
			If cInt(pQtde) > Session("Dia") Or cInt(pPerg) > Session("Dados") Then
				Response.Redirect "http://www.aggora.com.br/sabio/1/tema1/sabio03.asp?ptSala=" & pSala
				Response.End
			End If
		End If
		Set Perguntas = Server.CreateObject("ADODB.Recordset")
		Perguntas.Open "SELECT * FROM Perg0001 WHERE Codigo = " & pPerg, Conexao
			perg1 = Perguntas("Pergunta")
			figr1 = Perguntas("Figura")
			resp1 = Perguntas("a")
			resp2 = Perguntas("b")
			resp3 = Perguntas("c")
			resp4 = Perguntas("d")
	...
	Else
		pSala = 0
		Set Perguntas = Server.CreateObject("ADODB.Recordset")
		Perguntas.Open "SELECT * FROM Perg0001 WHERE Codigo = 1", Conexao
			perg1 = Perguntas("Pergunta")
			figr1 = Perguntas("Figura")
			resp1 = Perguntas("a")
			resp2 = Perguntas("b")
	...
	End If
		...
		Conexao.Execute "UPDATE UserDados SET USD_Sala1 = '" & text & "' WHERE USR_ID = " & Session("id")
	UsuarioDados.Close
	Set UsuarioDados = Nothing

%><Script Language="JScript" src="../images/Simples2.js"></Script>
<html><head><title>Sabichao</title><style>@import url(../images/coUC.css);</style></head>
<body bgcolor="#FFFFFF" onLoad="OnCarg()">
<p style="font-size: 10pt">
	Assunto da Sala: <font color="#0588C0"><b>OLIMPÍADAS, FUTEBOL, AUTOMOBILISMO</font><Span ID="NCalc"> - Carregando .... </Span></b>
...
<center>
<iframe src="../sabio17.asp" width="90%" style="border: 0px solid blue;"></iframe>
</center>
</body></html>