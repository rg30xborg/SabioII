...
function mudaB(){
    dadosc.action = "Sabio02.aspx"           //http://www.aggora.com.br/sabio/1/sorte/tema0/
	...
	var num1, num0, txtWeb = premio = ''; soundOK = "true"
...
function Aviso3(){
	//if((this.location.href).indexOf("http://www.aggora.com.br") < 0) location.href = "http://www.aggora.com.br/sabio/help.htm"
	//else{	// window.defaultStatus = 'RG 3.0 - Sabichão Web [GIGA SENNA 1]'
		NCalc.innerText = " "; Reg16.innerText = "Ok!!!!."
	//}
}

function alocar1(sndAction,sndObj){ 
	//sndObj = "document." + sndObj
	//if (eval(sndObj) != null){
   	...
   	if(cred > 0 && (cred - apost) >= 0){
		...
		if (gparc > 0){
			if(gparc == 6){ gparc = 10; myFirst[14] = ' <b>SENNA</b>'}
					...
					else if(gparc == 1){ gparc = apost * 10 / 100; myFirst[14] = ' <b>BIT-SENNA</b>'}
						else gparc = 0
		}
		myFirst[13] = gparc
		myFirst[15] = cred
...
function checkgroup(){
	...
	if(gparc > 0){ alocar1('play','sominici'); Reg17.innerHTML = 'Prêmio Simbólico: ' + (gparc == 10 ? pPremio : (gparc + ' Simulacros -')) + premio
...
function ProssRGSennaWeb(){  //SE FUNCIONAR ESTA FUNÇÂO E DEVERA SER TRANSFORMADA PARA ENVIAR A APOSTA
	if(cart == 0 || troc == 0){
		...
		txtWeb = 'V:' + '1' + 'G:' + grpsim + '#A:' + apost
			...
			//AQUI GOSTARIA DE TER UMA FUNCAO FLUSH...
                dadosc.action = "Sabio03.aspx"               //http://www.aggora.com.br/sabio/1/sorte/tema0/
				dadosc.salax.value = apost
				dadosc.salaz.value = grpsim
				top.frames.super1.Ponto.innerText = cred-apost
			dadosc.submit()

		}
		else { 
			cart = jogo = troc = 0; Reg21.src = '/_Library/botloc.gif'
		}
	}
}
