...
' Numero de perguntas no banco de dados na Sala e o ("hoje") é para computar o dia na Sala
	...
	Conexao.Open "DRIVER={MySQL ODBC 5.3 ANSI Driver};SERVER=187.45.196.225;PORT=3306;DATABASE=aggora1;USER=aggora1;PASSWORD=dade02;OPTION=3;"
		...
		UsuarioDados.Open "SELECT USD_Total, USD_Apts FROM UserDados WHERE USR_ID = " & Session("id"), Conexao
			libok = UsuarioDados("USD_Total")
			libSl = UsuarioDados("USD_Apts")
		UsuarioDados.Close
		Set UsuarioDados = Nothing

		If libok < 900 Or Session("xdemo") <> "demo01" Or Request.Form.Count <= 0 Then
		...
		Premio.Open "SELECT PRE_Cod, PRE_Desc, PRE_Data FROM Premios WHERE HEX(uuid) = '" & libHx & "'", Conexao
...
<p style="font-size: 10pt">
	...
	<span style="font-size: 8pt"><br>
	 - Premio: <font color="#0000FF"><span ID="Reg15"><%=libPr%></span> (em: <%=libDt%>)</font>
...
<table border="0" width="100%" cellspacing="0" cellpadding="0">
	...
	<tr>
		<td colspan=5 align=center width=100%><Script Language="JScript" SRC="/_Library/Defaull.js"></Script>
...
<table border="0" width="100%" cellspacing="0" cellpadding="0">
	<tr> 
		...
		com resultado obtido na apuração da Mega-Sena da Caixa (CEF) do <b>dia <%=libDt%></b>. Apostas neste prêmio estão liberadas até o
		<b>dia <%=DateAdd("d", -1, libDt)%></b> (um dia antes da apuração). Como Apostar -> Após selecionar o número de dezenas que terá
...
<%
	...
	Apostas.Open "SELECT * FROM Premios_Apostas WHERE USR_ID = " & Session("id") & " and PRE_Cod = " & libCd & " ORDER BY USA_Cod DESC", Conexao
...
%>
	<tr <%=bgcl%>>
		...
		<td align="center" nowrap><%=FormatDateTime(libDt, 2)%></td>
	</tr>
...
<form id="dadosc" name="dadosc" method="POST">
...
<audio id="somsorte" name="somsorte"><source src="/_Library/rgrodar.wav"></embed>
<audio id="somregis" name="somregis"><source src="/_Library/rgcaixa.wav"></embed>
<audio id="somclick" name="somclick"><source src="/_Library/rgclick.wav"></embed>
