    ...
    public partial class Sabio01 : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!IsPostBack)
            {
                ...
                using (Conexao)
                {
                    string queryString = "SELECT MAX(Codigo) AS max_Codigo FROM Perg0003";
                    ...
                    try
                    {
                        ...
                        queryString = "SELECT USD_Consec, USD_Sala3 FROM UserDados WHERE USR_ID = " + Session["id"];
                        ...
                        DadosS.Read();
                            ...
                            text = DadosS["USD_Sala3"].ToString();
                        DadosS.Close();
                        ...
                        queryString = "UPDATE UserDados SET USD_Sala3 = '" + text + "', USD_Acesso = '" + Hoje + "' WHERE USR_ID = " + Session["id"];
        ...
        protected void LeSala(OdbcConnection Conexao)
        {
            using (Conexao)
            {
                string queryString = "SELECT USD_Total, USD_Consec, USD_Sala3 FROM UserDados WHERE USR_ID = " + Session["id"];
                OdbcCommand command = new OdbcCommand(queryString, Conexao);
                ...
                try
                {
                    ...
                    UsuarioDados.Read();
                        text = UsuarioDados["USD_Sala3"].ToString();
        ...
        protected void NewPergunta(int IDPerg, OdbcConnection Conexao)
        {
            using (Conexao)
            {
                string queryString = "SELECT * FROM Perg0003 WHERE Codigo = " + IDPerg;
                OdbcCommand command = new OdbcCommand(queryString, Conexao);
                ...
                try
                {
                    ...
                    Perguntas.Read();
                        ...
                        cod   = Perguntas["Pos"].ToString();
                        fig   = Perguntas["Figura"].ToString();
                    Perguntas.Close();
                }
                catch (Exception ex)
                {
        ...
        protected void GravaSala(OdbcConnection Conexao)
        {
            using (Conexao)
            {
                string queryString = "UPDATE UserDados SET USD_Total = " + pTotl + ", USD_Consec = " + pCosc + ",  USD_Sala3 = '" + text + "', USD_Acesso = '" + Hoje + "' WHERE USR_ID = " + Session["id"];
                OdbcCommand command = new OdbcCommand(queryString, Conexao);
            ...
            using (Conexao)
            {
                if (HFPCert.Value == "0")
                {
                    ...
                    try
                    {
                        ...
                        {
                            perg1 = resp1 = resp2 = resp3 = resp4 = resp5 = fig = ""; cod = "0";
                            Session["livCav"] = 1;
                            if (IDPerg > slMax) IDPerg = 1;

                            //NewPergunta(IDPerg, Conexao);
                            queryString = "SELECT * FROM Perg0003 WHERE Codigo = " + IDPerg;
                            ...
                            Pergunta.Read();
                                ...
                                cod   = Pergunta["Pos"].ToString();
                                fig   = Pergunta["Figura"].ToString();
                            ...
                            //GravaSala(Conexao);
                            queryString = "UPDATE UserDados SET USD_Total = " + pTotl + ", USD_Consec = " + pCosc + ",  USD_Sala3 = '" + text + "', USD_Acesso = '" + Hoje + "' WHERE USR_ID = " + Session["id"];
                            command = new OdbcCommand(queryString, Conexao);
                            command.ExecuteNonQuery();

                            //return new Array(perg1,resp1,resp2,cod,fig)
                            //return new Array(perg1,resp1,resp2,cod,fig,resp3,resp4,resp5)
                            co[0] = perg1; HFco0.Value = perg1;
                            co[1] = resp1; HFco1.Value = resp1;
                            co[2] = resp2; HFco2.Value = resp2;
                            co[3] = cod;   HFco3.Value = cod;
                            co[4] = fig;   HFco4.Value = fig;
                            co[5] = resp3; HFco5.Value = resp3;
                            co[6] = resp4; HFco6.Value = resp4;
                            co[7] = resp5; HFco7.Value = resp5;
                        }
                        Conexao.Close();
                    }
                ...
                else if (HFPCert.Value == "1")
                {
                    ...
                    try
                    {
                        ...
                        if (valor > 0 && valor < 51 && Convert.ToInt16(Session["livCav"]) == 1)
                        {
                            Session["livCav"] = null;
