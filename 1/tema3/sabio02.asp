...
Else
	...
	Function NewPergunta(IDPerg, Conexao)
		...
		Perguntas.Open "SELECT * FROM Perg0003 WHERE Codigo=" & IDPerg, Conexao
			perg1 = Perguntas("Pergunta")
			resp1 = Perguntas("a")
			resp2 = Perguntas("b")
			cod = Perguntas("Pos")
			fig = Perguntas("Figura")
		Perguntas.Close
	...
	Function LeSala(Conexao)
		...
		UsuarioDados.Open "SELECT USD_Total, USD_Consec, USD_Sala3 FROM UserDados WHERE USR_ID = " & Session("id"), Conexao
			text0 = UsuarioDados("USD_Sala3")
	...
	Function GravaSala(Conexao)
		Conexao.Execute "UPDATE UserDados SET USD_Total = " & pTotl & ", USD_Consec = " & pCosc & ",  USD_Sala3 = '" & text1 & "' WHERE USR_ID = " & Session("id")
	End Function
End If
%>
...
function Method1(valor){
	...
	if(valor > 0 && valor < 51 && Session("livCav") == 1){
		Session("livCav") = null
...
function Method2(){
	...
	else{ perg1 = resp1 = resp2 = fig = "", cod = 0
		Session("livCav") = 1
